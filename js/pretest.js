...
async function loadNewQuestion(adjustment) {
    ...
    // nach letzter Frage auswerten und weiterleiten
...
/* -----------------------------
   Pretest-Auswertung für spätere Adaptivität
----------------------------- */
...
async function finalizePretest() {
    ...
    // lokal speichern, damit das adaptive WBT später darauf zugreifen kann
    localStorage.setItem("pretest_result", JSON.stringify(result));

    // optional auch als Gesamtevent in Supabase speichern
    await trackEvent({
        event_type: "pretest_result",
        page: "pretest",
        question_id: "pretest_summary",
        is_correct: null,
        attempts: null,
        duration_ms: null
    });

    // zusätzlich Detailwerte separat speichern
    localStorage.setItem("pretest_html_score", htmlScore);
    localStorage.setItem("pretest_css_score", cssScore);
    localStorage.setItem("pretest_js_score", jsScore);

    localStorage.setItem("pretest_html_level", result.html.level);
    localStorage.setItem("pretest_css_level", result.css.level);
    localStorage.setItem("pretest_js_level", result.js.level);

    redirectAfterPretest();
}
